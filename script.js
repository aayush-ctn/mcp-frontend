...
async function loginUser() {
  ...
  if (!name) {
    err.innerText = "Please enter your username.";
    return;
  }
  err.innerText = "";
  ...
  try {
    ...
    if (!token) {
      err.innerText = "No token returned from server.";
      return;
    }
...
async function createUser() {
  ...
  const err = document.getElementById("api-error");
  if (!name) {
    err.innerText = "Please enter a username to create an account.";
    return;
  }
  err.innerText = "";
  ...
  try {
    ...
    const data = await res.json();
    console.log(
      "[AUTH] Create user response status:",
      res.status,
      "| data:",
      data,
    );
    ...
    if (!res.ok) {
      err.innerText = data?.message || "Could not create account.";
      return;
    }
...
async function loadProjects() {
  ...
  if (projects.length === 0) {
    container.innerHTML =
      "<p class='empty-msg'>No projects yet — create one below.</p>";
    return;
  }
  projects.forEach((p) => {
...
async function loadTools() {
  ...
  if (tools.length === 0) {
    container.innerHTML =
      "<p class='empty-msg'>No tools yet — create one below.</p>";
    return;
  }
  tools.forEach((t) => {
...
async function createTool() {
  ...
  if (!res.ok) {
    alert("Error: " + (result?.message || "Failed to create tool"));
    return;
  }

  ["tool-name", "tool-description", "tool-url", "tool-headers"].forEach(
...
async function deleteTool(id) {
  ...
  const res = await fetch(`${BASE_URL}/api/tools/delete/${id}`, {
    method: "POST",
    headers: { token },
  });
  const data = await res.json();
...
async function setApiKeyAndGoToChat() {
  ...
  console.log(
    "[APIKEY] Calling set-api-key | userName:",
    userName,
    "| token1:",
    token ? "✓" : "✗",
  );
  ...
  try {
    ...
    if (!res.ok) {
      err.innerText = data?.message || "Failed to save API key.";
      return;
    }
    ...
    console.log(
      "[APIKEY] Token 2 (chat):",
      chatToken ? "stored ✓" : "MISSING ✗ — check setApiKey response shape",
    );

    if (!chatToken) {
      err.innerText = "No chat token returned from server.";
      return;
    }
    goToChat();
  } catch (e) {
...
async function loadChatTools() {
  ...
  const data = await res.json();
  const allTools = (data?.data?.tools || []).filter(
    (t) => String(t.project_id) === String(projectId),
  );
  console.log(
    "[CHAT-TOOLS] Status:",
  ...
  if (allTools.length === 0) {
    container.innerHTML =
      "<p class='empty-msg' style='font-size:12px'>No tools</p>";
    return;
  }
...
async function toggleChatTool(id, btn) {
  if (!sessionUuid) {
    alert(
      "Send a message first to start a session, then you can toggle tools.",
    );
    return;
  }
  ...
  try {
    ...
    if (res.ok) {
      const confirmed =
        typeof data?.data?.is_active === "boolean"
          ? data.data.is_active
          : optimistic;
...
const headerKeyHints = {
  ...
  api_key:
    "Check the API docs for the correct header name (e.g. X-Api-Key, apikey, api-key)",
...
function openAuthModal(toolId, toolName, authType, messageToResume) {
  // toolId, toolName, authType are resolved by the caller — cache lookup is a cross-check only.
  const toolData =
    chatToolsCache.find((t) => (t.id || t._id) === toolId) || null;
  pendingAuthToolId = toolId;
  // Caller-supplied toolName is authoritative; fall back to cache if somehow missing
  pendingAuthToolName = toolName || toolData?.name || "Unknown Tool";

  pendingAuthType = authType;
  pendingMessage = messageToResume || null;

  console.log(
    "[AUTH-MODAL] Opening | tool:",
    pendingAuthToolName,
    "| type:",
    authType,
    "| hasResume:",
  ...
  document.getElementById("auth-modal-title").innerText =
    `${authSaved ? "🔓 Update" : "🔒 Authenticate"}: ${pendingAuthToolName}`;
  ...
  document.getElementById("auth-modal-header-key").value =
    headerKeyDefaults[authType] || "Authorization";
  document.getElementById("auth-modal-header-hint").innerText =
    headerKeyHints[authType] || "";
...
async function confirmSaveToolAuth() {
  const headerKey = document
    .getElementById("auth-modal-header-key")
    .value.trim();
  const value = document.getElementById("auth-modal-value").value.trim();
  const err = document.getElementById("auth-modal-error");

  if (!headerKey) {
    err.innerText = "Please enter the header name.";
    return;
  }
  if (!value) {
    err.innerText = "Please enter the credential value.";
    return;
  }
  if (!pendingAuthToolId) {
    closeAuthModal();
    return;
  }
...
async function dispatchMessage(message) {
  ...
  try {
    ...
    if (detectAuthFailure(responseText)) {
      const responseToolId = data?.data?.toolId;

      // Prefer the exact tool the server identified as failing auth.
      // Only fall back to cache-scanning when the server didn't return a toolId.
      let toolToAuth = null;
      if (responseToolId) {
        toolToAuth = chatToolsCache.find(
          (t) => (t.id || t._id) === responseToolId,
        );
        if (!toolToAuth) {
          console.warn(
            "[CHAT] toolId from response not found in cache:",
            responseToolId,
          );
        }
      }
      if (!toolToAuth) {
        toolToAuth = findUnauthenticatedTool();
      }
      if (toolToAuth) {
        const tid = toolToAuth.id || toolToAuth._id;
        console.log(
          "[CHAT] Auth failure detected — opening modal for:",
          toolToAuth.name,
          "| toolId (from response):",
          responseToolId ?? "n/a (used cache fallback)",
        );
        // Small delay so bot message renders before modal
        setTimeout(
          () =>
            openAuthModal(tid, toolToAuth.name, toolToAuth.auth_type, message),
          500,
        );
      }
    }
    ...
    if (firstMessage && sessionUuid) {
      console.log(
        "[CHAT] First message — reloading tools with sessionUuid:",
        sessionUuid,
      );
      loadChatTools();
    }
